...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in range(num_classes):
        if(j != correct_class):
          loss = loss + np.maximum(0, 1 + score[j] - base_score)
    loss = loss / num_train
    pass
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in range(num_classes):
        if(j != correct_class):
          temp = np.maximum(0, 1 + score[j] - base_score)
          loss = loss + temp
          if(temp != 0):
            grad[j, :] = grad[j, :] + X[i, :]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    loss_matrix = np.maximum(0, 1 + score - base_score)
    loss_matrix[y, range(num_train)] = 0
    ...
    loss_matrix[y, range(num_train)] = -np.sum(loss_matrix, axis=0)
    grad = loss_matrix.dot(X) / num_train
