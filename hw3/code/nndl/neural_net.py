...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    # f = lambda x: np.exp(x - np.max(x, axis=0)) / np.sum(np.exp(x - np.max(x, axis=0)), axis=0)
    f = lambda x: x * (x > 0)
    h1 = f(X.dot(W1.T) + b1)
    scores = h1.dot(W2.T) + b2
    ...
    base_score = scores[y, range(N)]
    loss_matrix = -base_score
    loss_matrix = loss_matrix + np.log(np.sum(np.exp(scores), axis=0))
    ...
    # ================================================================ #
    dldh2 = np.exp(scores) / np.sum(np.exp(scores), axis=0)
    dldh2[y, range(N)] = dldh2[y, range(N)] - 1
    dldh2 = dldh2 / N
    grads['W2'] = dldh2.dot(h1)
    grads['W2'] = grads['W2'] + reg * W2
    grads['b2'] = np.sum(dldh2, axis=1)
    dldh1 = dldh2.T.dot(W2)
    dldh1[h1 <= 0] = 0
    grads['W1'] = dldh1.T.dot(X)
    grads['W1'] = grads['W1'] + reg * W1
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    f = lambda x: x * (x > 0)
    h1 = f(X.dot(self.params['W1'].T) + self.params['b1'])
