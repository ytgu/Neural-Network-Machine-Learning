...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    activations = (x - sample_mean) / np.sqrt(sample_var + eps)
    
    out = gamma * activations + beta

    cache = (activations, x - sample_mean, sample_var, eps, gamma)
    # cache = (x_normalized, x_wm, sample_var, eps, gamma)
    ...
    # ================================================================ #
    x_test_normalized = (x - running_mean) / np.sqrt(running_var + eps)
    out = gamma * x_test_normalized + beta
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  (x_normalized, x_wm, sample_var, eps, gamma) = cache
  N, D = dout.shape
  ...
  dxhat = dout * gamma
  dmiu = -np.sum(dxhat, axis=0) / np.sqrt(sample_var + eps)
  dvar = -0.5 * np.sum(x_wm * dxhat, axis=0) / (np.sqrt(sample_var + eps) * (sample_var + eps))
  term1 = dxhat / np.sqrt(sample_var + eps)
  term2 = 2 * x_wm * dvar / N
  term3 = dmiu / N
